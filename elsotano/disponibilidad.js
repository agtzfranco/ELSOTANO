document.addEventListener("DOMContentLoaded", () => {
  ...
  wrap.addEventListener("click", (e) => {
    ...
    if (!btn) return;
    state.servicio = SERVICIOS.find((s) => s.id === btn.dataset.id);
    state.fecha = null;
    state.hora = null;
    document
      .querySelectorAll(".bservice-card")
      .forEach((c) => c.classList.remove("selected"));
    btn.closest(".bservice-card").classList.add("selected");
  ...
  $("btnConfirmar").addEventListener("click", () => {
    ...
    if (!usuario) {
      window.location.href =
        "login.html?next=" + encodeURIComponent("disponibilidad.html");
      return;
    }
    ...
    fetch("api/reservar.php", {
      ...
      .then((data) => {
        ...
        if (data.ok) {
          ...
          $("mensaje").value = "";
          cargarSlots(); // refresca el horario ocupado
        } else {
          msg.style.color = "var(--error)";
