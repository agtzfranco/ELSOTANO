...
foreach (['servicio','fecha','hora'] as $campo) {
    if (empty($data[$campo])) { echo json_encode(['ok'=>false,'error'=>"Falta el campo: $campo"]); exit; }
}
...
if ($horaNum < 11 || $horaFinNum > 23) {
    echo json_encode(['ok'=>false,'error'=>'El horario debe estar entre 11:00 y 23:00.']); exit;
}
...
");
$stmt->execute([$servicio, $fecha, $horaFin, $horaInicio]);
if ($stmt->fetchColumn() > 0) {
    echo json_encode(['ok'=>false,'error'=>'Ese horario ya fue reservado. Elige otro.']); exit;
}
